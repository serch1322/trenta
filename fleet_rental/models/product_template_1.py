...
class ProductoProducto(models.Model):
    ...
    tipo_products = fields.Selection('product.template', related="tipo_product" , string="Tipo de Flota")

class registrarRecepcion(models.Model):
    _inherit = ['stock.picking']

    flota = fields.Boolean(string="Es Flota", default=False, store=True)
    ...
    def button_validate(self):
        ...
        for order in orden.order_line:
            if order.product_id.tipo_products == 'aditamento' or order.product_id.tipo_products == 'accesorio' or order.product_id.tipo_products == 'vehiculo':
                nombre_orden = {
                    'flota': True,
                }
            else:
